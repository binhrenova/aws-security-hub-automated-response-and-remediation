...
# Test the AWS CLI
account=`aws sts get-caller-identity --profile $profile | jq '.Account' | tr -d \"`
status=$?
if [ $status != 0 ]; then
...
# Validate region
region_check=`aws ec2 describe-regions --profile $profile --region $region | grep ec2.$region.amazonaws.com | wc -l`
status=$?
...
aws s3api head-bucket --profile $profile --bucket ${bucket}-reference --expected-bucket-owner ${account}
status=$?
if [ $status != 0 ]; then
...
aws s3api head-bucket --profile $profile --bucket ${bucket}-${region} --expected-bucket-owner ${account}
status=$?
if [ $status != 0 ]; then
...
aws s3 sync ./global-s3-assets s3://${bucket}-reference/$solution_name/$version/ --profile $profile
aws s3 sync ./regional-s3-assets s3://$bucket-${region}/$solution_name/$version/ --profile $profile
